...
Nx    = 30;     // cube side elements (x-dir, lengthwise)
Ny    = 30;     // cube height elements
Nz    =120;     // cube side elements (z-dir, widthwise)
Nout  = 60;     // elements out from cube
Nwake = 60;     // wake elements
...
// inner box points
Point(1) = {-L/2,0,-W/2,din};
Point(2) = {-L/2,0, W/2,din};
Point(3) = { L/2,0, W/2,din};
Point(4) = { L/2,0,-W/2,din};

Point(5) = {-L/2,H,-W/2,din};
Point(6) = {-L/2,H, W/2,din};
Point(7) = { L/2,H, W/2,din};
Point(8) = { L/2,H,-W/2,din};
...
Line( 5) = {6,7}; Transfinite Curve { 5} = Nx Using Bump Pc;
...
Line(30) = {5,8}; Transfinite Curve {30} = Nx Using Bump Pc;
Line(32) = {1,4}; Transfinite Curve {32} = Nx Using Bump Pc;

// define outer box lines
Line(11) = {9,10}; Transfinite Curve {11}  = Nz Using Progression 1;
Line(12) = {10,11}; Transfinite Curve {12} = Nx Using Progression 1;
Line(13) = {11,12}; Transfinite Curve {13} = Nz Using Progression 1;
Line(14) = {13,14}; Transfinite Curve {14} = Nz Using Progression 1;
Line(15) = {14,15}; Transfinite Curve {15} = Nx Using Progression 1;
...
Line(29) = {13,16}; Transfinite Curve {29} = Nx Using Progression 1;
Line(31) = {9,12}; Transfinite Curve {31}  = Nx Using Progression 1;
...
Physical Surface("ground") = {26, 10, 9, 11, 16, 29};
Physical Surface("cube")   = {1, 2, 3, 4, 15};
