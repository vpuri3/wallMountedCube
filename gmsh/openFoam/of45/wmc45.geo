...
Nx = 21;            // # points on cube side
Nz = 15;            // # points on cube side
...
Pc  = 1.00;          //   edge refinement on cube
Nsmooth = 0;         // # mesh smoothing iterations

Nd = 0.5*(Nz+1) - 3;

// fixed due to geom
Nbx = 0.5*(Nx+1);  // # points on side of small quads
Nbz = 0.5*(Nz+1);  // # points on side of small quads

Nzz = Nbx+Nbz+2*Nd-3; // # points in z-dir in wake and front region
...
// inner box points
Point(1) = {-L/2,0,-W/2,lc};
Point(2) = {-L/2,0, W/2,lc};
Point(3) = { L/2,0, W/2,lc};
Point(4) = { L/2,0,-W/2,lc};

Point(5) = {-L/2,H,-W/2,lc};
Point(6) = {-L/2,H, W/2,lc};
Point(7) = { L/2,H, W/2,lc};
Point(8) = { L/2,H,-W/2,lc};

Rotate {{0, 1, 0}, {0, 0, 0},-Pi/4} { Point{ 1:8 } ; }
...
Line(2) = {2,3}; Transfinite Curve {2}   = Nx Using Bump Pc;
Line(3) = {3,4}; Transfinite Curve {3}   = Nz Using Bump Pc;
Line(4) = {4,1}; Transfinite Curve {4}   = Nx Using Bump Pc;

// cube upper
Line(5) = {5,6}; Transfinite Curve {5}   = Nz Using Bump Pc;
Line(6) = {6,7}; Transfinite Curve {6}   = Nx Using Bump Pc;
Line(7) = {7,8}; Transfinite Curve {7}   = Nz Using Bump Pc;
Line(8) = {8,5}; Transfinite Curve {8}   = Nx Using Bump Pc;
...
Line(145) = {57,58}; Transfinite Curve {145} = Nzz;
Line(146) = {59,60}; Transfinite Curve {146} = Nzz;
...
Line(153) = {61,62}; Transfinite Curve {153} = Nzz;
Line(154) = {63,64}; Transfinite Curve {154} = Nzz;
...
Physical Surface("ground") = {91,96,6:9,31:42};
Physical Surface("cube")   = {1:5};
